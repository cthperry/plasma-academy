 ...
 * 棘輪基線。
 *
 * 標記前是 35 / 242 = 14 %。上一輪把每章「首次出現」的術語補上標記
 * (docs/09 明文的規則),覆蓋率到 115 / 242 = 48 %,基線設在 45 %。
 *
 * 2026-08 這一輪(docs/13 §5(a)、`tools/mark-glossary-terms.mjs`)把
 * 「課文有中文但沒標記」的 59 條裡,47 條機械式補完,覆蓋率到
 * 163 / 247 = 66 %。剩下 12 條(如「電子伏特」「Bohm 速度」「Townsend
 * 雪崩」)故意沒補——它們在全站唯一的出現位置是 `.pa-formula__name`
 * 這類公式標題或標題(h1–h6),插標記會讓 `gen-formulas.mjs` 用來配對
 * 章節錨點的純文字比對失準(踩過一次,見該腳本的檔頭註解),
 * 寧可少標 12 條也不要弄壞公式手冊的錨點。
 * 基線設在 63 %,留一點餘裕給日後課文改寫;
