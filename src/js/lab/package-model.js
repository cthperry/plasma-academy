...
(function (PA) {
  ...
  /* ====================================================================
     PCB 除膠渣(desmear)與回蝕(etchback)

     這一段與上面的封裝活化是**同一章的兩個世界**,而且結論剛好相反:
     3.7.2 說「封裝端幾乎不用氟系」,PCB 的 desmear 卻**非加 CF₄ 不可**。
     原因只有一個 —— **玻纖**。

     FR-4 = 環氧樹脂 + 玻璃纖維布。鑽孔時摩擦生熱把樹脂熔開、抹在孔壁上
     (smear),擋住內層銅與後續化學銅的接觸。O₂ 電漿灰化得掉樹脂,
     但**對玻纖(SiO₂)完全無效** —— 樹脂被挖深、玻纖原地不動,
     結果是一根根「玻纖突出」戳在孔壁上,鍍銅照樣包不住。
     要讓玻纖一起退,必須有 F:SiO₂ + 4F → SiF₄↑(揮發)。

     所以模型的核心是**兩條互相競爭的移除速率**:

       樹脂  ∝ O 的比例 ×(1 + a·CF₄ 比例)   ← 少量 F 反而加速有機物移除
       玻纖  ∝ CF₄ 比例                      ← 沒有 F 就完全不動

     樹脂那條的括號項是真實效應(F 先抽走 H、把高分子鏈打開,讓 O 更容易
     氧化),與 3.7.2 的 Ar+O₂ 協同是同一類的「離子/自由基互相幫忙」。
     但 CF₄ 加太多會把 O 稀釋掉,所以樹脂速率**先升後降** ——
     又是一條先升後降的曲線,和本章 A33 的接著力同一個形狀、同一個教訓。

     a = 5/3 讓樹脂速率的峰值落在 CF₄ = 20 %,與現場常用的 10–25 % 一致。

     ⚠️ 速率的絕對值是教學用的典型量級(µm/min),不是任何特定機台的
     實測值;模型要守住的是**定性結論**:純 O₂ 必然留下玻纖突出、
     加 CF₄ 有一個最佳點、加過頭兩件事同時變糟。
     這幾條由 tools/check-package.mjs 斷言。
     ==================================================================== */
  ...
  /** 樹脂移除速率 [µm/min] */
  function resinRate(cf4frac, flux) {
    var x = Math.min(1, Math.max(0, cf4frac));
    return RESIN_BASE * (1 - x) * (1 + RESIN_SYNERGY * x) * flux;
  }

  /** 玻纖移除速率 [µm/min] —— 沒有 F 就是 0 */
  function glassRate(cf4frac, flux) {
    var x = Math.min(1, Math.max(0, cf4frac));
    return GLASS_BASE * x * flux;
  }
  ...
  function desmear(opts) {
    var cf4 = Math.min(1, Math.max(0, opts.cf4 == null ? 0.2 : opts.cf4));
    var P = opts.power_W == null ? 300 : opts.power_W;
    var pr = opts.pressure_Torr == null ? 0.4 : opts.pressure_Torr;
    var t = Math.max(0, opts.time_min == null ? 15 : opts.time_min);
    var target = opts.target === "etchback" ? "etchback" : "desmear";
    var flux = radicalFlux(P, pr, "lp");

    var resin = resinRate(cf4, flux) * t;
    var glass = glassRate(cf4, flux) * t;
    var protrusion = resin - glass;

    // 目標深度:desmear 只要清掉抹層,etchback 要刻意讓樹脂退到露出內層銅三面
    var window = target === "etchback" ? [12, 25] : [3, 8];
    var okDepth = resin >= window[0] && resin <= window[1];
    // |突出| 超過 1.5 µm 現場就會在切片上看得出來、鍍層可靠度開始掉
    var okFlush = Math.abs(protrusion) <= 1.5;

    var verdictText;
    if (cf4 === 0) verdictText = "玻纖完全沒退 —— 純 O₂ 去不掉 SiO₂,孔壁會留下玻纖突出";
    else if (protrusion > 1.5) verdictText = "玻纖突出 —— CF₄ 比例不足,樹脂退得比玻纖快";
    else if (protrusion < -1.5) verdictText = "玻纖被過度咬蝕 —— CF₄ 過量,樹脂速率也被稀釋掉";
    ...
    return {
      ...
      resinRate: resinRate(cf4, flux),
      glassRate: glassRate(cf4, flux),
      window: window,
      okDepth: okDepth,
      okFlush: okFlush,
      verdict: verdictText,
