...
export const defectKnobs = ["壓力", "Source 功率", "Bias 功率", "聚合性氣體", "O₂", "溫度", "時間", "脈衝", "遮罩"];
...
export function filterDefects(options = {}) {
  return defects.filter((defect) => {
    if (options.cat && defect.cat !== options.cat) return false;
    if (options.knob && !defect.fixes.some((fix) => fix.knob === options.knob)) return false;
    if (options.q) {
      const query = String(options.q).toLowerCase();
      const haystack = `${defect.zh}${defect.en}${defect.symptom}${defect.causes.join("")}${defect.distinguish}`.toLowerCase();
  ...
  /** 依旋鈕反查:動這個旋鈕會影響哪些缺陷 */
export function defectsByKnob(knob) {
  return defects.filter((defect) => defect.fixes.some((fix) => fix.knob === knob));
}
